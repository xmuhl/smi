...
## 当前状态（2026-08-28 · GitHub 调度全天丢弃 + 延迟补发脏数据事故）

- **09-01 事故与修复**：GitHub 调度第 3 次大面积延迟/丢弃（08-27/08-28 同源）——close-snapshot 16:23/18:23/19:23、archive-raw 16:35、watchdog 17:10~20:10、t1 18:17/20:17 傍晚窗口全部未按时到达（迟到触发 21:32/21:37 才到，周一窗口凌晨补发）；watchdog 15:58（自身迟到触发）检出 SITE_LATEST_STALE 并 dispatch 恢复，但恢复运行 15:59 撞上 16:00 前的 BEFORE_CLOSE 守卫（exit 3→0）空转，傍晚 watchdog 窗口同样被丢 → 21:01/21:14/21:19 手动 dispatch 才恢复。同日 t1-reconcile 四连败（runner 海外 IP 被交易所拒：非 JSON 响应/连接重置），margin 缺口 08-28/08-31。修复：本机 t1 回补两日 FINAL（9ad003e，环比链 -60.09/+56.00 补齐）+ dispatch 重采部署（d07cc80，09-01 margin latestRef 刷至 08-31）；watchdog 修订——today 巡检 16:00 CST 前视为预期未发布，良性跳过不派发不误报（9719a46，含离线回归测试）。**教训：16:00 前的自动恢复注定被守卫吞掉，恢复派发必须与守卫阈值对齐**。飞书 secret 仍未配置，本次红灯仅 GitHub 页面可见（PENDING §1 第三次提醒）

- **08-28 事故与修复**：GitHub cron 当日全丢（08-27 事故升级版，连 freshness-watchdog 自身也未触发——**同平台第二 cron 不是兜底**）；且昨夜被延迟 7~10h 补发的窗口在凌晨执行造成次生损害：archive-raw 03:14 盘前运行把 THS「即时」表的 08-27 收盘值打成 08-28 标签写入 7 行脏归档（450fd9a），close-snapshot×3 因 BEFORE_CLOSE 与 VALIDATION_FAILED 共用 exit 2 产生假红灯，t1 margin 回补因 runner IP 重置真失败。修复：本机 t1 回补 margin 08-27 FINAL + 清 7 行脏归档（98a77f9）；**盘前守卫**——close_snapshot BEFORE_CLOSE 改 exit 3（workflow shell 内转 0，假红灯消除；exit 2 语义完整保留，子代理评审 P1 修正：不用 continue-on-error）+ archive_raw 新增 16:00 CST 同阈守卫防错日脏数据（d9e6e58）；恢复 = push → dispatch archive-raw（70a033c，written=7）→ dispatch close-snapshot。**教训：dispatch 走 API 免疫 cron 丢弃；凡 --date auto 的 job 必须盘前守卫；「即时」类接口盘前返回昨日值是数据源语义陷阱**。完整分析 work/INCIDENT_20260828_RECOVERY.md；跨平台检查 runbook（待人工接入调度）ops/AUTOCHECK_RUNBOOK.md；人工裁决项 work/PENDING_HUMAN_DECISIONS.md（FEISHU secret 第三次提醒 / 跨平台看门狗三选一 / 迁移选项）
- **08-27 事故与修复（存档）**：GitHub 调度全天丢弃 5+ 个 cron 触发（close-snapshot×3 窗口 + archive-raw + t1×2 晨窗；workflow 均 active、cron 未变，08-26 同调度正常——属 GitHub 侧调度丢弃）。**"未触发"不产生失败运行 → GitHub 通知零告警**，20:19/20:38 手动 dispatch 才恢复发布（7/9 模块 FINAL）。archive-raw 手动补跑（20:57/21:02）晚于 close-snapshot → tracks 缺当日涨停池 → 全赛道 limitUpCount/连板缺列 → coverage 57.6<65 fail-closed UNAVAILABLE。修复：`freshness-watchdog` 看门狗上线（盘后 17:10~20:10 CST 巡检线上 latest.json，滞后自动 dispatch close-snapshot + 飞书/红灯；次晨 10:40 catchup 对上一交易日缺失自动 dispatch manual-backfill；dispatch 是 API 调用，免疫 cron 丢弃；45/90 分钟派发冷却防叠加）——08-28 证明其同平台依赖是盲区
...
- **待观察**：① 08-28 恢复日 close-snapshot 重建质量（tracks 应有当日前5，非 UNAVAILABLE）② THS board-close 日线盘后延迟生成（20:15 仍 DATE_NOT_FOUND，08-27 记录为 22:00 后才可得）→ 当日 close 序列缺行由次日 collect_board_close_history 回补 ③ FEISHU_WEBHOOK_URL secret 仍未配置（PENDING_HUMAN_DECISIONS §1）④ 跨平台看门狗待人工接入（ops/AUTOCHECK_RUNBOOK.md）
...
| `freshness-watchdog.yml` | Cron 工作日 17:10/18:10/19:10/20:10 + 次晨 10:40 CST | **cron 丢弃兜底**：巡检线上 latest.json / 上一交易日文件，滞后即自动 dispatch close-snapshot（盘后）/ manual-backfill（次晨）+ 红灯飞书；dispatch 走 API 免疫调度丢弃（2026-08-27 事故）；**today 巡检 16:00 CST 前触发（调度延迟补发）视为预期未发布，良性跳过不派发不误报**（2026-09-01 修订——此前迟到触发会派发被 BEFORE_CLOSE 守卫空转的恢复并误报红灯） |
